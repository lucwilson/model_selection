%% ms-SPRiNT (without Brainstorm)
% Inputs 
F = xs;                             % Input time series
RowNames = {'channel_1'};           % Input time series channel names
% STFT opts
...
SPRiNT = struct('opts',opt,'freqs',Freqs,'channel',channel);
% Parameterize STFTs
SPRiNT_output = SPRiNT_ms_specparam(TF, Freqs, ts, opt, RowNames);
...
%%  Step 2: Parameterize spectrograms spectra using model selection
function SPRiNT = SPRiNT_ms_specparam(TF, FreqVector, ts, opt, RowNames)
...
% Find all frequency values within user limits
    ...
    for chan = 1:nChan % use parfor here for parallel
    ...
    SPRiNT.TimeVector = ts'; % Reassign times by windows used
...
function ys = build_model(freqs, ap_pars, ap_type, pk_pars, peak_function)
...
%           Model power spectrum, in log10-space
    ...
    if length(pk_pars) > 1
        for peak = 1:size(pk_pars,1)
            ys = ys + peak_function(freqs,pk_pars(peak,1),...
